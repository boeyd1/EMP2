           ...
           let data = UIImageJPEGRepresentation(productImage.image!, 0.1)
            ...
            DBProvider.Instance.imageStorageRef.child(currentUserId + "\(NSUUID().uuidString).jpg").put(data!,metadata: nil) { (metadata: FIRStorageMetadata?, err: Error?) in
                ...
                if err != nil {
                    ...
                    DBProvider.Instance.createInventory(senderID: currentUserId, shopName: currentShopName!, name: self.productNameTF.text!, description: self.productDescTV.text, price: self.productPriceTF.text!,quantity: self.productQuantityTF.text!, url: String(describing: metadata!.downloadURL()!))
